...
const variableGroups = [
  {
    label: 'Palette',
    vars: [
      { name: '--palette-name', label: 'Palette Name', desc: 'Used in CSS variable names.', type: 'text', default: 'custom' },
      { name: '--color-primary-550', label: 'Primary Color', desc: 'Main brand color (550 shade).', type: 'color', default: '#0172ad' },
      { name: '--color-primary-350', label: 'Primary Light', desc: 'Lighter variant for hover and dark mode text.', type: 'color', default: '#01aaff' },
      { name: '--color-primary-on-fill', label: 'Text on Primary', desc: 'Text color over primary backgrounds.', type: 'color', default: '#ffffff' },
    ]
  },
  {
    label: 'Core Colors',
...
function getScope(v) {
  if (v.name === '--palette-name') return 'root'
  const d = v.default || ''
...
function downloadCSS() {
  const paletteNameValue = customValues['root:--palette-name'] || customValues['--palette-name'] || 'custom'
  const name = String(paletteNameValue).trim().toLowerCase().replace(/[^a-z0-9_-]/g, '') || 'custom'
