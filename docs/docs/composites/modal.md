...
3. Create an opening button with `command="show-modal" commandfor="your-modal-id"` (alternatively, you can use JS `document.querySelector('#your-modal-id').showModal()`)
4. The modal will display with a backdrop and trap focus automatically
5. Close the modal through the use of a button with `command="close"` (alternatively, you can use JS `document.querySelector('#your-modal-id').close()`)
