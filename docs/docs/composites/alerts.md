...
> **Warning:** Be mindful that the role `role="alertdialog"`, it tells the browser to trigger a system alert event, which immediately announces the contents to the user and ensures they focus on the prompt, so it's important to consider the use of this role or simply use a normal [Modal](/docs/composites/modal) for less critical messages.
