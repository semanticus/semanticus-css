...
> **Important:** Assistive technologies implicitly add the `role="main"` so, don't combine it with `role="group"` or any other roles like `role="region"` that would override the main landmark semantics.
