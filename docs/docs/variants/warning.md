...
Apply `.warning` to `<div>` elements with `.pane` for a warning action pane.

<HtmlPreviewer :code="WarningDemo.panes()" />

## Panels

Apply `.warning` to `<div>` elements with `.panel` for a warning action panel.

<HtmlPreviewer :code="WarningDemo.panels()" />
