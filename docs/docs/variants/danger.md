...
Apply `.danger` to `<div>` elements with `.pane` for a danger action pane.

<HtmlPreviewer :code="DangerDemo.panes()" />

## Panels

Apply `.danger` to `<div>` elements with `.panel` for a danger action panel.

<HtmlPreviewer :code="DangerDemo.panels()" />
