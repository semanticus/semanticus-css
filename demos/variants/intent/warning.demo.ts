...
export function links(attrs: Record<string, string> = {}) {
  return renderElement('nav', {}, `${ADemo.main({ ...attrs, class: ["warning", attrs.class || ""].join(' ') })}

${RoleLinkDemo.main("button", { ...attrs, class: ["warning", attrs.class || ""].join(' ') })}

${RoleLinkDemo.main("div", { ...attrs, class: ["warning", attrs.class || ""].join(' ') })}`);
}

export function buttons(attrs: Record<string, string> = {}) {
  return renderElement('nav', { role: 'toolbar' }, `${ButtonDemo.main({ ...attrs, class: ["warning", attrs.class || ""].join(' ') })}

${RoleButtonDemo.main("div", { ...attrs, class: ["warning", attrs.class || ""].join(' ') })}

${RoleButtonDemo.dropdown({ ...attrs, class: ["warning", attrs.class || ""].join(' ') })}`);
}
